.goPairSim = function(
  twoid, golist = golist,
  ont = ont, organism = organism, measure = measure, combine = combine) {
...
#' Protein Sequence Similarity Calculation based on Gene Ontology
#' (GO) Similarity
#'
...
parGOSim = function(
  ...
  if ( type == 'go' ) {
    ...
    for ( i in 1:ncol(idx) ) {
      gosimlist[[i]] = .goPairSim(
        rev(idx[, i]), golist = golist,
        ont = ont, organism = organism, measure = measure, combine = combine)
    }
...
twoGOSim = function(
  ...
  ont = 'MF', organism = 'human', measure = 'Resnik', combine = 'BMA') {

  if ( type == 'go' ) {
    sim = GOSemSim::mgoSim(
      id1, id2, ont = ont, organism = organism,
